...
<!-- PROJECT LOGO -->
<h1>Constant Product AMM Solidity Contract</h1>
...
An Automated Market Maker (AMM) is a decentralized financial protocol that facilitates the trading of digital assets on blockchain networks, such as Ethereum.

We built an `AMM` smart contract, that depends on constant product algorism (X * Y = K).
...
1. Clone the repo
   ```sh
   git clone https://github.com/Al-Qa-qa/constant-product-amm.git
   ```
2. Install NPM packages (remember to move to the directory `constant-product-amm` first)
   ```sh
   npm install
   ```
...
Project Link: [https://github.com/Al-Qa-qa/constant-sum-amm](https://github.com/Al-Qa-qa/constant-product-amm)
